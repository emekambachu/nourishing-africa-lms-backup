...
var iti = window.intlTelInput(input, {
    ...
    onlyCountries: ["us","gb","dz","ao","bj","bw","bf","bi","cm","cv","cf","td","cd","cg","ci","dj","eg","gq","er","et","ga","gm","gh","gn","gw","ke","ls","lr","ly","mg","mw","ml","mr","mu","ma","mz","na","ne","ng","rw","rs","sy","rc","ri","rw","st","sn","sc","sl","so","za","ss","sd","sz","tz","tg","tn","ug","zm","zw"],
...
function preliminaryQuestion(question, val){
    ...
    if(question == 'q1'){
       q1 = val;
        ...
        if(q1 == 'yes'){
            $('#stage-area-checkbox-1').removeClass('stage-area-checkbox-active');
    ...
    if(question == 'q2'){
        ...
        if(q2 == 'yes')
        {
            $('#stage-area-checkbox-2').removeClass('stage-area-checkbox-active');
    ...
    if(question == 'q3'){
        ...
        if(q3 >= '25' && q3 <= '40')
        {
            ...
            $('#stage-area-checkbox-3').removeClass('stage-area-checkbox-active');
    ...
    if(q1 == 'yes' && q2 == 'yes' && q3 == 'yes' && ( q4 == 'yes' || q4 == 'no')){
        $('#info-text').text('Sign Up');
        $('#stage-area-checkbox-intro').removeClass('d-none');
    }
    else if(question === 'q4' && (q1 === 'no' ||  q2 === 'no' || q3 === 'no') && (q4 === 'yes' || q4 === 'no')){
        $('#stage-area-checkbox-backhome').removeClass('d-none');
    }
}
...
function CompanyLegal(val){
    if(val == "1"){
        $("#legalPane").removeClass('d-none');
    }
    else{
        $("#legalPane").addClass('d-none');
...
var name; var lname; var fname; var email; var id_card; var confirm_email; var dob; var phone; var state_origin; var state_residence; var education_level; var gender;  var globalBool;
var password; var password_confirmation;

function isEmpty(str) {
    return (!str || str.length === 0 || /^\s*$/.test(str));
}

function emailExist(boolvalue){
    $.ajaxSetup({
        headers:{
            'X-CSRF-TOKEN' : $('meta[name="csrf-token"]').attr('content')
        }
    ...
    $.ajax({
        ...
        beforeSend : function()
        {
            if(!isEmpty(email) && ValidateEmail(email)){
                $('#email-part-1-fb').text('Validating email, pls wait ...');
            }
        },
        ...
        error: function(e, textStatus) 
        {
            if(textStatus==="timeout") {
                $('#email-part-1-fb').text('Please refresh your browser to allow us verify your email.');
            } 
        }
...
function processFile(document){
    if(document){
        ...
        if($.inArray(doc_ext, ['jpg', 'jpeg','png','pdf','doc','docx']) === -1) {
...
function referralevent(){
    ...
    if(val == 'other' || val == 'From another organisation' || val == 'Invitation from a Nourishing Africa Team Member'){
...
function stagecontentpart1form(){
    ...
    /*if(this.fname != "" && this.lname != ""){
        $("#name-part-1").val(this.fname.trim()+" "+this.lname.trim())
    }*/
    ...
    this.id_card = $('#id-card-part-1').val();
    if(isEmpty(this.id_card)){
        invalidFeedbackPart('1','id-card', 'This field is required');
        boolvalue += 1;
    }else{
        validFeedbackPart('1','id-card');
    }

    this.confirm_email = $('#confirm-email-part-1').val();
    if(isEmpty(this.confirm_email)){
        invalidFeedbackPart('1','confirm-email','Confirm email field is required');
        boolvalue += 1;
    }else{
        if(!ValidateEmail(this.confirm_email)){
            ...
            if(this.email !== this.confirm_email){
                invalidFeedbackPart('1','confirm-email','Email Addresses does not match!');
                boolvalue += 1;
            }
        }
    }
    ...
    if(isEmpty(this.phone)){
        ...
        if (this.input.value.trim()) {
            if (this.iti.isValidNumber()) {
                ...
                invalidFeedbackPart('1','phone', errorMap[errorCode]);
                boolvalue += 1;
            }
        }
    }
    ...
    if(isEmpty(this.state_residence)){
        invalidFeedbackPart('1','state-residence', 'State of residence is required');
        boolvalue += 1;
    }else{
    ...
    if(isEmpty(this.education_level)){
        invalidFeedbackPart('1','education-level', 'Higest level of education is required');
        boolvalue += 1;
    }else{
    ...
    if(isEmpty(this.password)){
        ...
        if(this.password.length < 6){
            invalidFeedbackPart('1','password', 'Minimum of six characters is required');
            boolvalue += 1;
        }else{
    ...
    if(isEmpty(this.password_confirmation)){
        invalidFeedbackPart('1','password-confirmation', 'Password confirmation is required');
        boolvalue += 1;
    }else{
        if(this.password_confirmation.length < 6){
            invalidFeedbackPart('1','password-confirmation', 'Minimum of six characters is required');
            boolvalue += 1;
        }else{
    ...
    if(!isEmpty(this.password) && !isEmpty(this.password_confirmation) && this.password.length >= 6 && this.password_confirmation.length >= 6) {
        if(this.password === this.password_confirmation){
            validFeedbackPart('3','password');
            validFeedbackPart('3','password-confirmation');
        }else{
            invalidFeedbackPart('3','password-confirmation', 'Password does not match');
            invalidFeedbackPart('3','password', 'Password does not match');
            boolvalue += 1;
        }
    }
    
    emailExist(boolvalue);
}
...
var business_name; var registering_as; var busstop_landmark; var biz_location; var value_chain; var business_running; var reg_number; var business_association;
var business_association_name; var referral_type; var referral_other; var vc_other; var loc_other; var fa_other; var br_other; var reg_number_doc;
var business_address; var company_type; var focus_area; var company_stage; var tags; var company_description;
var website; var def_social_media_link; var def_social_media_link_2; var def_social_media_link_3; var def_social_media_link_4;
...
function stagecontentpart2form(){
    ...
    if(isEmpty(this.business_address)){
        invalidFeedbackPart('2','business-address', 'Business address is required');
        boolvalue += 1;
    }else{
    ...
    if(isEmpty(this.website) && isEmpty(this.def_social_media_link) && isEmpty(this.def_social_media_link_2) && isEmpty(this.def_social_media_link_3) && isEmpty(this.def_social_media_link_4)){
        boolvalue += 1;
...
var business_description, business_operation, number_employees, number_ft_employees, number_pt_employees, number_women_employees, export_interest, buisness_revenue_projection;
var new_jobs_projection, job_creation_summary, business_ownership_stake, business_revenue, disabilities, disability_type, export_license, referral_type, referral_other;
...
function stagecontentpart3form(){
    ...
    if(isEmpty(this.business_description)){
        invalidFeedbackPart('3','business-description', 'This field is required');
        boolvalue += 1;
    }else{
        var res = this.business_description.split(" ");
        if(res.length < 10 || res.length > 100){
            invalidFeedbackPart('3','business-description', 'Minimum of 10 words and maximum of 100 words.');
            boolvalue += 1;
        }else{
    ...
    if(isEmpty(this.business_operation)){
        invalidFeedbackPart('3','business-operation', 'This field is required');
        boolvalue += 1;
    }else{
    ...
    if(isEmpty(this.number_women_employees)){
        invalidFeedbackPart('3','number-women-employees', 'This field is required');
        boolvalue += 1;
    }else{
    ...
    if(isEmpty(this.disability_type)){
        if(this.disabilities == "1"){
            invalidFeedbackPart('3','disability-type', 'This field is required');
            boolvalue += 1;
        }else{
            $('#disability-type-part-3').removeClass('is-invalid');
            $('#disability-type-part-3-fb').removeClass('invalid-feedback');
    ...
    if(isEmpty(this.referral_other) && this.referral_type == 'From another organisation'){
    ...
    if(boolvalue === 0){
        ...
        if(!confirm("Are you sure you would like to go ahead with the submission.")){
            return false;
        }else{

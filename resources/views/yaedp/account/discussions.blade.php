...
@section('content')
    <div class="container mb-4">
        ...
        <p class="bread-crumbs">
            <a href="{{ route('yaedp.account') }}">Dashboard</a> / <a href="{{ route('yaedp.account.modules') }}">Modules</a>
            / <a href="{{ route('yaedp.account.courses', $course->learningModule->id) }}">Courses</a> / <span
                    class="light-green">{{ $course->title }}</span>
        </p>
            ...
            <div class="col-lg-9 col-md-9 col-sm-12">
                <div class="card h-100 w-100">
                    <div class="card-header border-bottom">
                        <label class="learning-discussion-header-title" for=""><span style="color: #D75C03;"> Discussion - </span>({{ $course->title }}
                            )</label>
                        <br/>
                        <label class="learning-discussion-header-comment-fig mb-0 mt-3" for="">{{ count($discussion) }}
                            Comment{{ count($discussion) > 1 ? 's':'' }}</label>
                        <button class="float-right discussion-reply-btn" data-toggle="modal"
                                data-target="#exampleModalCenter" data-type="Comment">Comment
                        </button>
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6 col-sm-12">
                                                        <div class="float-right">
                                                            <button id="comment-submit-btn" class="comment-form-submit">
                                                                <img class="pr-1 d-none" id="submit-img"
                                                                     src="{{ asset('images/floading.gif') }}">Submit
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                @foreach($discussion as $item)
                                    <div class="row mb-3">
                                        <div class="col-12">
                                            ...
                                            <label for=""
                                                   class="pl-3 discussion-comment-name">{{ \App\Models\YaedpUser::getUserFullName($item->user_id) }}</label>
                                            <label for=""
                                                   class="float-right discussion-comment-time">{{ Carbon\Carbon::parse($item->created_at)->diffForHumans(null, true).' ago' }}</label>
                                        </div>
                                        <div class="col-12 mt-2">
                                            <p class="discussion-comment-body">{{ $item->message }}</p>
                                        </div>
                                        <div class="col-12">
                                            <button class="discussion-comment-like" data-commentid="{{ $item->id }}"
                                                    data-type="comment">
                                                <img id="commentlike{{ $item->id }}" style="width: 16px; height: 16px;"
                                                     src="{{ \App\Models\Learning\Discussion\LearningDiscussionLike::check("comment", $course->id, $item->id) ? asset("images/icons/chkdfav.png") : asset("images/icons/like.png") }}"
                                                     alt="">
                                                &nbsp;&nbsp; Like
                                                ({{ \App\Models\Learning\Discussion\LearningDiscussionLike::countLikes("comment", $course->id, $item->id) }}
                                                )
                                            </button>
                                            <button class="discussion-comment-reply"
                                                    data-commentdiv="subcomment{{ $item->id }}">
                                                Replies({{ \App\Models\Learning\Discussion\LearningDiscussionReply::getCount($item->id) }}
                                                )
                                            </button>
                                            <button class="discussion-comment-reply float-right"
                                                    ...
                                                    data-target="#exampleModalCenter"><img
                                                        src="{{asset("images/icons/reply.png")}}" alt="">&nbsp;&nbsp;
                                                Reply
                                            </button>
                                        </div>
                                    </div>
                                                    ...
                                                    <div class="row pl-3 mb-3">
                                                        <div class="col-12">
                                                            <img src="{{ asset('images/icons/profile.png') }}" alt=""
                                                                 class="">
                                                            <label for=""
                                                                   class="pl-3 discussion-comment-name">{{ \App\Models\YaedpUser::getUserFullName($replies->user_id) }}</label>
                                                            <label for=""
                                                                   class="float-right discussion-comment-time">{{ Carbon\Carbon::parse($replies->created_at)->diffForHumans(null, true).' ago' }}</label>
                                                        </div>
                                                        <div class="col-12 mt-2">
                                                        ...
                                                        <div class="col-12">
                                                            <button class="discussion-comment-like"
                                                                    data-commentid="{{ $item->id }}" data-type="reply"
                                                                    data-replyid="{{ $replies->id }}">
                                                                <img id="replylike{{ $replies->id }}"
                                                                     style="width: 16px; height: 16px;"
                                                                     src="{{ \App\Models\Learning\Discussion\LearningDiscussionLike::check("reply", $course->id, $item->id, $replies->id) ? asset("images/icons/chkdfav.png") : asset("images/icons/like.png") }}"
                                                                     alt="">
                                                                &nbsp;&nbsp; Like
                                                                ({{ \App\Models\Learning\Discussion\LearningDiscussionLike::countLikes("reply", $course->id, $item->id, $replies->id) }}
                                                                )
                                                            </button>
                                                            <button class="discussion-comment-reply float-right"
                                                                    data-type="Reply"
                                                                    data-directreplyid="{{ $replies->id }}"
                                                                    data-commentid="{{ $item->id }}" data-toggle="modal"
                                                                    data-target="#exampleModalCenter"><img
                                                                        src="{{asset("images/icons/reply.png")}}"
                                                                        alt="">&nbsp;&nbsp; Reply
                                                            </button>
                                                        </div>
                                                    </div>
                ...
                @if($course->nextCourse($module->id))
                    ...
                    @if(Auth::user()->startedCourse($course->id, $module->id))
                        ...
                        @elseif(Auth::user()->startedCourse($course->id, $module->id)->status === 1 && !$course->nextCourse($module->id))
                            <a class="next-course-link"
                               href="{{ route('yaedp.account.assessment.start', $module->id) }}">
                                <button class="module-btn bg-light-brown d-flex justify-content-center">
                                    Start Assessment
                                </button>
                            </a>
